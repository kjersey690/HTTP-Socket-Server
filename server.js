...
const server = net.createServer((request)=>{
 ...
 request.on('data', data =>{
  ...
  let requestArray = httpRequest.split("\n");
  let requestLine = requestArray[0].split(' ');
  var serverName = "localHost";
  let method = requestLine[0];
  let path = requestLine[1];
  let spec = requestLine[2];
  let today= "Today's date : "+ new Date().toUTCString();
...
switch(path){
  ...
  // console.log(spec);
  request.write('spec');
  // request.write(" " + spec + "200 OK" +"\n"+ "server"+":"+serverName+"\n"+today +" "+ "\n"+"Content-Length:"+ indexHTML.length+ "\n"+"\n"+ indexHTML);
    break;

  // case "/index.html":
  // request.write(spec+"200 OK"+ today+" "+"\n" + path + "\n"+" "+ indexHTML.length+ "\n"+"\n"+ indexHTML);
  //   break;
...
var styles = `@import url(http://fonts.googleapis.com/css?family=Open+Sans|Roboto+Slab);
